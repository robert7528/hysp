...
- **微前端**: `micro-app` iframe 模式（見 5.14 微前端架構決策）。
...
| `-ui` | 後台 UI（micro-app 子應用，掛載於 hyadmin-ui） | Next.js/Bun | `hycert-ui` |
| `-pui` | 前台 UI（Portal/Public，面向用戶） | Next.js/Bun | `hysso-pui` |
...
#### 方案選定：micro-app iframe 模式
- **框架**：`@micro-zoe/micro-app`
- **模式**：`iframe`（物理隔離，每個子應用獨立 window）
- **父應用**：hyadmin-ui（Next.js App Router），作為 Shell 掛載子應用

#### 為什麼選 iframe 模式

| 模式 | 問題 |
|--|--|
| 預設模式（JS 沙箱） | 不支援 ESM — micro-app 用 `new Function()` 包裝 JS，與 Vite/Next.js 的 ESM 輸出不相容 |
| inline 模式 | Next.js RSC streaming 的 ReadableStream 與 inline 的 eval 衝突 |
| inline + disableSandbox | ESM 可行，但多子應用同頁時無 JS 隔離（常用功能頁籤場景需要隔離） |
| **iframe 模式** | **完全隔離，所有框架都相容，多子應用共存安全** |
...
| 前進/後退 | 無 — micro-app iframe 模式內建路由同步 | 已內建（query parameter 機制） |
| 資源重複載入 | 暫無（一次一個子應用） | HTTP 快取優化 + 切換時銷毀 iframe |

#### POC 驗證記錄（2026-03-18）
- 預設模式 + Vite：**失敗**（`Cannot use import statement outside a module`）
- inline + disableSandbox + Vite：**成功**（但不適用於多子應用同頁場景）
- iframe + Next.js：**成功**（已部署驗證，hycert-ui 工具箱可載入操作）

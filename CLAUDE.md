...
| `-ui` | 前台 UI | 純 Next.js/Bun | `hycert-ui` |
| `-mui` | 後台 UI（micro-app，m=management） | 純 Next.js/Bun | `hycert-mui` |

- 有前台+後台：建 3 個（`-api` + `-ui` + `-mui`）
- 只有後台：建 2 個（`-api` + `-mui`）
- 只有前台：建 2 個（`-api` + `-ui`）

### 5.3 平台中控台：hyadmin
HySP Console 是 HySP 平台的管理介面，由 hyadmin 實作。負責租戶管理、模組管理、系統設定，並作為微前端 Shell 掛載各模組的 `-mui`。維持現有命名不改名。

| Repo | 角色 | Slug |
|--|--|--|
| `hyadmin-api` | 中控台 API | `hub` |
| `hyadmin-ui` | 中控台 Shell（掛載各模組 -mui） | `hub` |
...
- **建立時機**：第二個 API 模組開工前，從 hyadmin-api 抽取。
...
3. **命名與 Slug**：依命名規則（`hy{模組}-api` / `-ui` / `-mui`）命名，指定 URL slug

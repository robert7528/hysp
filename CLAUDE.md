...
**參數分層與優先級**：租戶覆蓋 > 模組專屬 > 平台通用 > 程式碼 fallback

| 層級 | 範例 | 說明 |
|--|--|--|
| 平台通用 | `platform.default_page_size = 20` | 所有模組共用 |
| 模組專屬 | `hycert.cert_list.page_size = 15` | 覆蓋平台值 |
| 租戶覆蓋 | tenant + key → value | 特定租戶覆蓋 |
| 程式碼 fallback | 前端/後端常數 | hyconf 不可用時使用 |

與 5.9 i18n 分層邏輯一致。現階段各模組先用常數管理，hyconf 上線後替換為 API。

...
### Backend (Golang)
- **架構**: `uber-go/fx` (DI), `Gin` (Web), `Cobra/Viper` (CLI/Config)。
- **Gateway**: 待選型（KrakenD / Kong 等），目前由 Nginx 代理。
- **穩定性**: `go-zero/core` (mr, GoSafe, breaker), `x/sync/errgroup`（視需要引入）。
- **資安**: `casbin` (RBAC/ABAC), `google/tink` (加密), `HashiCorp Vault` (KMS)。
- **資料庫**: `PostgreSQL` (JSONB), `Redis`, `GORM` (dbresolver), `ElasticSearch`。
- **測試**: `stretchr/testify` (斷言), `uber-go/mock` (Mock 生成), `jarcoal/httpmock` (HTTP Mock)。
- **快取**: `eko/gocache`（統一快取介面，待 Redis 整合時引入）。
...
- **微前端**: `wujie-react`（iframe 模式，見 5.13 微前端架構決策）。
...
| Repo | 角色 | Slug |
|--|--|--|
| `hyadmin-api` | 中控台 API | `hyadmin` |
| `hyadmin-ui` | 中控台 Shell（掛載各模組 -ui） | `hyadmin` |
...
| 模組 | Slug | 功能 |
|--|--|--|
| `hyadmin` | `hyadmin` | 中控台 Shell、租戶管理、模組管理 |
| `hysso` | `hysso` | 認證（JWT/SSO/OAuth/SAML）+ 身份管理（使用者檔/管理者檔/帳密綁定） |
| `hyiam` | `hyiam` | 授權 / 權限管理（Casbin policy、角色管理，可掛載到各模組選單 `?module=xxx`） |
| `hyconf` | `hyconf` | 通用參數管理（可掛載到各模組選單 `?module=xxx`） |
| `hylog` | `hylog` | 操作日誌（可掛載到各模組選單 `?module=xxx`） |

#### hylog 演進規劃

目前操作日誌由 `hycore/auditlog` middleware 直接寫入 tenant DB，查詢 API 在 hyadmin-api。

| 階段 | 時機 | 做法 |
|--|--|--|
| **現階段** | 模組 ≤ 2 個 | 維持現狀：hycore middleware → 直接寫 DB，hyadmin-api 提供查詢 |
| **拆分** | 第 3 個業務模組出現 | 抽出 hylog-api 獨立服務，各模組改打 hylog-api 寫入 |
| **擴展** | 日誌量大時 | hylog-api 後端接 ElasticSearch，支援全文搜尋與大量儲存 |

關鍵設計：`hycore/auditlog` middleware 介面不變，後端從直接寫 DB 改成打 API，各模組不用改 code。
...
```
/hyconf/settings?module=hycert    ← 只顯示 hycert 的參數
/hyiam/roles?module=hycert        ← 只顯示 hycert 的權限
/hylog/audit?module=hycert        ← 只顯示 hycert 的日誌
```

#### hyconf 參數管理設計
集中服務 + 模組各自註冊，程式碼內保留 fallback 預設值（模組可獨立運行）。

**參數分層與優先級**（三層，與 i18n 一致）：

| 優先級 | 層級 | tenant_id | 範例 |
|--|--|--|--|
| 1 | 租戶覆蓋 | `system` 等 | `hycert.cert_list.page_size = 50`（客戶客製） |
| 2 | 平台預設 | `__default__` | `platform.default_page_size = 20`、`hycert.cert_list.page_size = 15` |
| 3 | 程式碼 fallback | — | 前端 `constants.ts` / 後端 Go 常數 |

- **平台預設**一層內靠 key 前綴區分 scope（`platform.` 通用、`hycert.` 模組專屬），不是額外的優先級層。
- **租戶覆蓋**只存差異（跟平台預設不同的 key），不需要完整一份。
- **程式碼 fallback** 是完整一份，hyconf 不可用時保底，本來就存在於程式碼中。
- 現階段各模組先用程式碼 fallback（常數），hyconf 上線後替換為 API。

### 5.5 API 路由分層
單一 API 服務內，以路徑區分前後台與共用：

```
/{slug}-api/api/v1/adm/...    ← 後台 API（adminAuth + casbin middleware）
/{slug}-api/api/v1/pub/...    ← 前台 API（userAuth middleware）
/{slug}-api/api/v1/...        ← 共用 API（基本 auth middleware）
```

- 各層掛不同 middleware group 控制權限。
- `{slug}` 為模組名稱（與 repo 前綴一致，如 `hyadmin`、`hycert`），Nginx path 為 `/{slug}-api/`。
- Nginx trailing slash strip 前綴後轉發，應用程式碼內部統一使用 `/api/v1/...`，不含 slug（部署環境無關）。
...
Nginx 路由規則（Podman Nginx / K8S Ingress 皆適用）：
```
/{slug}-api/* → strip /{slug}-api → 轉發至對應的 API 服務
/{slug}-ui/*  → strip /{slug}-ui  → 轉發至對應的 UI 容器 nginx
/{slug}/      → hyadmin-ui 專用（Shell，不加 -ui 後綴）
```

範例：
```
/hyadmin-api/api/v1/health → http://127.0.0.1:8081/api/v1/health
/hycert-api/api/v1/health  → http://127.0.0.1:8082/api/v1/health
/hyadmin/                  → hyadmin-ui container (SPA fallback)
/hycert-ui/                → hycert-ui container (SPA fallback)
```
...
#### 多租戶資料分層（三層，與 hyconf 一致）

| 優先級 | 層級 | 存在哪 | 說明 |
|--|--|--|--|
| 1 | 租戶覆蓋 | `i18n_messages`（tenant_id = 租戶） | 只存差異 key，客戶客製翻譯 |
| 2 | 平台預設 | `i18n_messages`（tenant_id = `__default__`） | 管理者維護的翻譯 |
| 3 | 程式碼 fallback | 靜態 JSON（`en.ts` / `zh-TW.ts`） | 打包進 App，零 latency |

- key 前綴區分 scope：`shared.`（跨模組共用）、`hycert.`（模組專用），同一層不同 scope
- 租戶只存跟平台預設不同的 key，不需要完整一份
...
3. **命名與 Slug**：依命名規則（`hy{模組}-api` / `-ui` / `-pui`）命名，slug = 模組名稱（如 `hycert`），Nginx path = `/{slug}-api/`、`/{slug}-ui/`
...
| 組合 | 定位 | 包含模組 |
|--|--|--|
| **HySP 完整版** | 全功能平台 | hyadmin + hysso + hyiam + hyconf + hylog + 業務模組 |
| **HySP Lite** | 輕量管理平台 | hyadmin + hylog |
| **模組獨立版** | 單一功能產品 | hysso + 業務模組（如 hylog、hycert） |

#### 獨立運行的認證方案
目前所有模組的認證依賴 hyadmin-api 簽發 JWT。獨立運行時搭配 `hysso`：
- hysso 負責登入/認證/JWT 簽發
...
- **多租戶 DB**：憑證 CRUD、CSR、部署管理、ACME 帳戶/訂單存 tenant DB；Agent Token 存 admin DB
- **Auth**：JWT 驗證（人用，共用 hyadmin-api 的 JWT_SECRET）+ Agent Token（機器用，SHA-256 hash）
- **格式支援**：PEM、DER、PFX/PKCS#12、JKS（keystore-go/v4）、P7B/PKCS#7（smallstep/pkcs7）
- **鏈驗證**：AIA chasing + AKID/SKID + SystemCertPool
- **ACME**：go-acme/lego（Let's Encrypt / ZeroSSL），DNS-01（Cloudflare/Manual）+ HTTP-01
- **排程**：robfig/cron 每日自動續約掃描
...
- **模組註冊**：在 hyadmin 中設定 `route: 'hycert'`, `url: 'https://domain/hycert-ui'`, `api_url: '/hycert-api'`
